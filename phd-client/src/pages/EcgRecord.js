...
import { getEcgRecordRequest } from '../actions/ecgActions';
import config from '../config';

class EcgRecord extends Component {
  ...
  componentDidMount(){
    AsyncStorage.multiGet(['username', 'patientName', 'patientAge', 'patientMobile', 'patientAddress', 'patientId'], (err, result)=> {
      this.setState({
          ...
          getEcgRecordRequest(this.state.patientId).then((res)=> {
            this.setState({
...
render(){
  const columns = [ 'Heart Rate', 'Result', 'Date',  'Report'];
  const columnWidth = [100, 125, 75, 60];

  let dataSource = this.mapping();

  const element = (data, index) => (
    <TouchableOpacity onPress={() => this.getImage(data)}>
      <View>
    ...
    <Text style={styles.secondaryText}>Patient Details are</Text>
      ...
      <Text style={styles.patientText}>Mobile: {this.state.patientMobile}</Text>
              ...
              dataSource.map((rowData, index) => (
                <TableWrapper key={index} style={styles.row}>
                {
                  rowData.map((cellData, cellIndex) => (
                    <Cell key={cellIndex} data={cellData != undefined ? cellIndex == 3 ? element(cellData, index) : cellData : "No data"} width= {columnWidth[cellIndex]}/>
                  ))
                }
                </TableWrapper>
              ))
        ...
        <Button
          ...
          onPress={()=>{ this.props.navigation.navigate('EcgSummary')}}
        />
     </ ScrollView>
    </ BackgroundImage>
