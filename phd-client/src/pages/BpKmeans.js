...
class BpKmeans extends Component {
  ...
  setPatientDetails(){
      AsyncStorage.multiGet(['username', 'patientName', 'patientAge', 'patientMobile', 'patientAddress', 'patientId']).then((result)=>{
        this.setState({
          ...
          getBpRecordRequest(this.state.patientId).then((result)=> {
            let filterRecord =[];
            result.data.records.forEach((val,index)=> {
              filterRecord.push([val.lower_bp, val.higher_bp])
            });
            this.setState({
  ...
  mappingKmeans(){
    let result = kmeans.array2d(this.state.records,4);
    let filterRecord=[];
...
render(){
  let chartData=[];
  let data = [];
  if(this.state.records.length > 8){
    data = mappingKmeans();
